<template>
  <div class="simulation-fight" v-show="visible">
    <div class="container">
      <div class="tabs">
        <dl :class="{ curr: tabIndex === 0 }" @click="changeTab(0)">
          已有怪物模拟
        </dl>
        <dl :class="{ curr: tabIndex === 1 }" @click="changeTab(1)">
          自定义怪物模拟
        </dl>
      </div>
      <div class="panel" v-show="tabIndex === 0">
        <div class="enemy-info">
          <dl>
            ...
            <span
              >胜率：{{
                panel[0].fightNum > 0
                  ? Math.floor(
                      (panel[0].victoryNum1 / panel[0].fightNum) * 1000
                    ) / 10
                  : 0
              }}%</span
            >
          ...
          <dl>
            <label for="">HP：</label
            ><be-hp-bar
              :curr="panel[0].enemy1.currHp"
              :max="panel[0].enemy1.hp"
            ></be-hp-bar>
          </dl>
        </div>
        <div class="enemy-info">
          <dl>
            ...
            <span
              >胜率：{{
                panel[0].fightNum > 0
                  ? Math.floor(
                      (panel[0].victoryNum2 / panel[0].fightNum) * 1000
                    ) / 10
                  : 0
              }}%</span
            >
          ...
          <dl>
            <label for="">HP：</label
            ><be-hp-bar
              :curr="panel[0].enemy2.currHp"
              :max="panel[0].enemy2.hp"
            ></be-hp-bar>
          </dl>
        </div>
        <div class="config config0">
          <dl>
            ...
            <select v-model="panel[0].enemyType1">
              <option
                :key="'enemyType-' + enemyType.value"
                v-for="enemyType in enemyTypes"
          ...
          <dl>
            ...
            <select v-model="panel[0].enemySuffix1">
              <option
                :key="'enemySuffix-' + enemySuffix.value"
                v-for="enemySuffix in enemySuffixs"
                :value="enemySuffix.value"
              >
                {{ enemySuffix.label }}
              </option>
            </select>
        ...
        <div class="config config0">
          <dl>
            ...
            <select v-model="panel[0].enemyType2">
              <option
                :key="'enemyType2-' + enemyType.value"
                v-for="enemyType in enemyTypes"
                :value="enemyType.value"
              >
                {{ enemyType.label }}
          ...
          <dl>
            ...
            <select v-model="panel[0].enemySuffix2">
              <option
                :key="'enemySuffix2-' + enemySuffix.value"
                v-for="enemySuffix in enemySuffixs"
                :value="enemySuffix.value"
              >
                {{ enemySuffix.label }}
      ...
      <div class="panel" v-show="tabIndex === 1">
        <div class="enemy-info">
          <dl>
            ...
            <span
              >胜率：{{
                panel[1].fightNum > 0
                  ? Math.floor(
                      (panel[1].victoryNum1 / panel[1].fightNum) * 1000
                    ) / 10
                  : 0
              }}%</span
            >
          ...
          <dl>
            <label for="">HP：</label
            ><be-hp-bar
              :curr="panel[1].enemy1.currHp"
              :max="panel[1].enemy1.hp"
            ></be-hp-bar>
          </dl>
        </div>
        <div class="enemy-info">
          <dl>
            ...
            <span
              >胜率：{{
                panel[1].fightNum > 0
                  ? Math.floor(
                      (panel[1].victoryNum2 / panel[1].fightNum) * 1000
                    ) / 10
                  : 0
              }}%</span
            >
          ...
          <dl>
            <label for="">HP：</label
            ><be-hp-bar
              :curr="panel[1].enemy2.currHp"
              :max="panel[1].enemy2.hp"
            ></be-hp-bar>
          </dl>
        </div>
...
export default {
  ...
  methods: {
    ...
    initEnemy(tabIndex = 0) {
      ...
      switch (tabIndex) {
        case 0:
          this.panel[0].enemy1 = new Enemy(
            this.panel[0].enemyType1,
            this.panel[0].enemySuffix1,
            this.panel[0].lv1
          );
          this.panel[0].enemy2 = new Enemy(
            this.panel[0].enemyType2,
            this.panel[0].enemySuffix2,
            this.panel[0].lv2
          );
          break;
        case 1:
    ...
    fight(cb) {
      ...
      let atkFn1 = () => {
        var ch = this.panel[this.tabIndex].enemy1.attack(
          this.panel[this.tabIndex].enemy2
        );
        if (ch.currHp <= 0) {
          this.clearSto();
          this.$message({
      ...
      this.panel[this.tabIndex].sto1 = setTimeout(atkFn1, 1);
      let atkFn2 = () => {
        var ch = this.panel[this.tabIndex].enemy2.attack(
          this.panel[this.tabIndex].enemy1
        );
        if (ch.currHp <= 0) {
          this.clearSto();
          this.$message({
