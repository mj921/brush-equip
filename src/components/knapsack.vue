    ...
    <div class="container">
      <div class="already-equipped">
        <dl v-for="key in Object.keys(player.equips)" :key="key">
          <template v-if="player.equips[key]">
            <span
              :style="'color:' + player.equips[key].getColor()"
              @click="showEquipDetail(player.equips[key], 'unload')"
              >{{ player.equips[key].getName() }} (lv:
              {{ player.equips[key].lv }})</span
            >
      ...
      <div class="not-equip">
        <dl v-for="(equip, i) in player.knapsack" :key="i">
          <span
            :style="'color:' + equip.getColor()"
            @click="showEquipDetail(equip, 'equip')"
            >{{ equip.getName() }} (lv: {{ equip.lv }})</span
          >
          <span
            class="red-text"
            v-if="player.getCurrEquipPower(equip) > player.getCombatPower()"
            >+{{
              player.getCurrEquipPower(equip) - player.getCombatPower()
            }}</span
          >
        ...
        <div class="mult-sale-container" @click="stopProp($event)">
          <template v-for="equipQuality in equipQualityList">
            <dl :key="equipQuality.code">
              <input
                type="checkbox"
                v-model="equipQuality.value"
                :id="'multSale' + equipQuality.code"
              /><label :for="'multSale' + equipQuality.code">{{
                equipQuality.name
              }}</label>
            </dl>
          </template>
  ...
  methods: {
    showEquipDetail(equip, btnType) {
      if (btnType === 'equip') {
        ...
        let powerUp =
          this.player.getCurrEquipPower(equip) - this.player.getCombatPower();
    ...
    multSale() {
      let saleQuality = this.equipQualityList
        .filter((item) => item.value)
        .map((item) => item.code);
      if (saleQuality.length > 0) {
        let saleEquips = this.player.knapsack.filter(
          (equip) => saleQuality.indexOf(equip.quality) > -1
        );
        saleEquips.forEach((equip) => {
          this.sale(equip);
        });
      }
      this.closeMultSale();
    ...
    intelligenceSale() {
      let equips = this.player.knapsack.filter(
        (equip) =>
          this.player.getCurrEquipPower(equip) < this.player.getCombatPower()
      );
      equips.forEach((equip) => {
    ...
    burnish(equip) {
      let burnishPrice = equip.getBurnishPrice();
