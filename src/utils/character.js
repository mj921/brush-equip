...
export default class Character {
    ...
    if (Math.random() * 100 < this.hit && Math.random() * 100 >= enemy.dodge) {
      ...
      if (this.baseSkill) {
        if (this.baseSkill.damageType === DamageType.Physical) {
          damage =
            this.baseSkill.getDamage(
              Math.random() * (this.maxAtk - this.minAtk) + this.minAtk
            ) *
            (1 - enemy.getDamageRemission(this));
        } else {
          damage =
            this.baseSkill.getDamage(this.magicAtk) *
            (1 - enemy.getMagicDamageRemission(this));
        }
      ...
      obj.msg = `<span style='color: ${this.color || '#F67712'};'>${
        this.name
      }</span> 使用 ${skillName} 对 <span style='color: ${
        enemy.color || '#F67712'
      };'>${enemy.name}</span> 造成 <span style='color: #F32020;${
        crtFlag ? 'font-size: 1.4em;' : ''
      }'>${damage}</span> ${damageName}`;
    } else {
      obj.msg = `<span style='color: ${enemy.color || '#F67712'};'>${
        enemy.name
      }</span> 闪避了 <span style='color: ${this.color || '#F67712'};'>${
        this.name
      }</span> 的攻击`;
    }
  ...
  getCombatPower() {
    return Math.floor(
      (((((((((((this.maxAtk + this.minAtk) / 2 + this.magicAtk) /
        2 /
        this.interval) *
        (this.crt + 100) *
        this.crtDamage) /
        ...
        this.hp) /
        (2 -
          this.getDamageRemissionByCombatPower() -
          this.getMagicDamageRemissionByCombatPower())) *
        2) /
        (100 - this.dodge)) *
        30
    );
  ...
  getOnHookDamage(enemy) {
    ...
    if (!this.baseSkill || this.baseSkill.damageType === DamageType.Physical) {
      damage = Math.floor(
        ((((((this.maxAtk + this.minAtk) / 2) *
          (1 - enemy.getDamageRemission(this)) *
          this.hit) /
          100) *
          (100 - enemy.dodge)) /
          100) *
          (1 + (this.crt * this.crtDamage) / 10000)
      );
    } else {
      damage = Math.floor(
        ((((this.magicAtk *
