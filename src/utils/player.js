...
export default class Player extends Character {
  ...
  getEquips(equips) {
    ...
    if (knapsack.length > this.knapsackCapacity) {
      ...
      knapsack
        .slice(this.knapsackCapacity, knapsack.length)
        .forEach((equip) => {
  ...
  calculationBaseAttr() {
    ...
    this.magicDef = Math.floor(
      (this.baseMagicDef * (100 + this.extraMagicDef)) / 100
    );
    this.magicAtk = Math.floor(
      (this.baseMagicAtk * (100 + this.extraMagicAtk)) / 100
    );
